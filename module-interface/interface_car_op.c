...
void interface_car_op_text(const struct database *db, const index client_i)
{
        ...
        printf("[2] Auto adatainak modositasa\n");
        printf("[3] Auto eltavolitasa\n");
        printf("[4] Javitas/vizsga hozzadasa\n");
        printf("[5] Javitas/vizsga adatainak modositasa\n");
        printf("[6] Javitas/vizsga eltavolitasa\n");
        printf("-------------------------------------------------------\n");
        ...
        while (submenu_active) {
                ...
                switch (opt) {
                        ...
                        case 2:
                                ...
                                response = interface_car_modify(db, client_i, opt);
                        ...
                        case 5:
                                ...
                                response = interface_op_modify(db, client_i, opt,
                                        opt2);
                                if (response == ERR_OUT_OF_RANGE)
...
int interface_car_modify(const struct database *db, const index client_i,
        const index car_i)
{
        char name_buffer[NAME_MAX_LEN + 1] = "\0";
        char plate_buffer[PLATE_MAX_LEN + 1] = "\0";
        ...
        return db_modify_car(db, client_i, car_i, name_buffer, plate_buffer);
}

int interface_op_modify(const struct database *db, const index client_i,
        const index car_i, const index op_i)
{
        ...
        printf("Javitas koltsege (forintban): ");
        interface_io_fgets_clean(price_buffer, DEFAULT_BUFFER_SIZE + 1);
        sscanf(price_buffer, "%lf", &price);

        return db_modify_op(db, client_i, car_i, op_i, desc_buffer, price, NULL);
}
